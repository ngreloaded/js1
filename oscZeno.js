...
if(inBox.val().indexOf("Infinity")>-1 || inBox.val().indexOf(strMathError)>-1) return ; 
    ...
    function oscBinaryOperation() {
        ...
        switch (opCode) {
            ...
            case 6: stackVal = Math.pow(stackVal, x2); break;
	...
	if(inBox.val().indexOf("Infinity")>-1 || inBox.val().indexOf(strMathError)>-1) return ; 
        switch (this.id) {
            // +/-                                 
            case 'keyPad_btnInverseSign': retVal = -x; stackVal2 = 3; break;
            // 1/X                                 
            ...
            // SIN                                 
            case 'keyPad_btnSin': retVal = sinCalc(modeSelected,x); modeText($("#"+this.id).text(),x);break;
            // COS                                 
            case 'keyPad_btnCosin': retVal = cosCalc(modeSelected,x);modeText($("#"+this.id).text(),x); break;
            // TAN                                 
            case 'keyPad_btnTg': retVal = tanCalc(modeSelected,x); modeText($("#"+this.id).text(),x);break;
            // CTG                                 
            ...
            //Log Base 2
            case 'keyPad_btnLogBase2': retVal = Math.log(x)/Math.log(2);displayTrignometric("logXbase2",x);break;
                
            //###### Added By Harsh : Ends  #####//                              
                
                
            // Arcsin                                
            case 'keyPad_btnAsin': retVal = sinInvCalc(modeSelected,x);modeText("asin",x); break;
            // Arccos                                
            case 'keyPad_btnAcos': retVal = cosInvCalc(modeSelected,x); modeText("acos",x);break;
            // Arctag                                
            case 'keyPad_btnAtan': retVal = tanInvCalc(modeSelected,x);modeText("atan",x); break;
    ...
    $("div#keyPad a.keyPad_btnCommand").click(function () {
        ...
        switch (this.id) {
			...
			opCodeArray.pop();
				...
				if(stackVal1 != 2) { if(stackVal1 == 3 ||stackVal2 == 1) {strInput = ""; } inBox1.val(inBox1.val()+strInput);}
				stackVal1 = 2;
				newOpCode = 0;
